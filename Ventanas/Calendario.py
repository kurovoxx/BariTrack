...
class Calendario(New_ventana):
    def __init__(self, panel_principal, color):
        ...
        self.mensage("Selecciona una fecha para agregarla a la base de datos.", "Calendario")
        self.cargar_fechas_resaltadas()  # Cargar las fechas al iniciar

    def add_widget_calendario(self):
        # Crear calendario
        self.calendario = Calendar(self.panel_principal, selectmode='day', date_pattern='yyyy-mm-dd', showweeknumbers=False)
    ...
    def guardar_fecha(self):
        ...
        try:
            ...
            # Crear tabla de fechas si no existe
            cursor.execute("""
            ...
            cursor.execute("INSERT INTO fechas_seleccionadas (fecha) VALUES (?)", (fecha_seleccionada,))
            conn.commit()

            CTkMessagebox(title="Fecha Guardada", message=f"Fecha {fecha_seleccionada} guardada con éxito.", icon='info', option_1="Ok")
            ...
            CTkMessagebox(title="Error", message="Hubo un error al guardar la fecha.", icon='error', option_1="Ok")
    ...
    def resaltar_fecha(self, fecha):
        ...
        self.calendario.calevent_create(fecha_datetime, "Fecha Seleccionada", tags='resaltada')  # Resalta la fecha
        self.calendario.tag_config('resaltada', background='green', foreground='white')
    ...
    def cargar_fechas_resaltadas(self):
        ...
        try:
            ...
            CTkMessagebox(title="Error", message="Hubo un error al cargar las fechas.", icon='error', option_1="Ok")
