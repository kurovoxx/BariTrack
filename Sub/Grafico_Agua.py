...
class Grafico_Agua(Grafico_Calorias):
    def obtener_datos(self, usuario):
        query = """SELECT fecha, cant FROM agua GROUP BY fecha ORDER BY strftime('%Y-%m-%d', substr(fecha, 7, 4) || '-' || substr(fecha, 4, 2) || '-' || substr(fecha, 1, 2))"""
    ...
    def crear_grafico(self, frame, datos):
        ...
        self._configurar_ejes(ax, 'Agua vs Tiempo', 'Agua')
        
        if len(agua) > 0:
            bars = ax.bar(fecha, agua, color=verde_alegre, edgecolor='black', linewidth=1.5)
            for bar in bars:
